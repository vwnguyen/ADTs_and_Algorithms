...
// #: 1467315
// Assignment: pa4
// Description: Source file for Graph ADT implementation
//-----------------------------------------------------------------------------
...
Graph newGraph(int n){
    ...
    for(int i=0; i<=n; i++){
		...
		G->vertex[i].discoverTime = NIL;
		G->vertex[i].finishTime = NIL;
    }
...
void Visit(int x, Graph G, int* time, List S){
	*time = *time + 1;
	G->vertex[x].discoverTime = *time; // discover X, 0 -> 1
	G->vertex[x].color = GRAY;
	//printf("vertex: %d color: %d\n",x,G->vertex[x].color);
	//printf("beginning of Visit(), time: %d\n",*time);
	// the length of the adjacency list is not 0
	if ( length(G->vertex[x].adjacencyList) != 0 ){
		// for all y in adj[x]
		moveFront(G->vertex[x].adjacencyList);	
		while(index(G->vertex[x].adjacencyList) != UNDEF){
			// 2
			int checking = get(G->vertex[x].adjacencyList);
			//printf("checking: %d\n",checking);
			if ( G->vertex[checking].color == WHITE){
				G->vertex[checking].parent = x;
				Visit(checking,G,time,S);
			}
			moveNext(G->vertex[x].adjacencyList);			
		}
		
		
		G->vertex[x].color = BLACK;
		*time = *time + 1;
		//printf("end of Visit(), time: %d\n",*time);
		G->vertex[x].finishTime = *time; // finish X
		append(S,x);
...
void DFS(Graph G, List S){
	...
	if( length(S) == getOrder(G)  ){
		// initialize all vertices to undiscovered and no parent
		for ( int i = 1; i <= getOrder(G); i++ ){
			G->vertex[i].color = WHITE;
			G->vertex[i].parent = NIL;
		}
		int initialLength = length(S);
		time = 0;
		moveFront(S);
		while ( index(S) != UNDEF ) {
			// process all undiscovered vertices in the graph
			if ( G->vertex[get(S)].color == WHITE ) {
				// delete(S);
				// vertex is an array of vertex info objects
				//printf("visiting %d\n",get(S));
				//moveFront(G->vertex[get(S)].adjacencyList);
				Visit(get(S),G,&time,S); 
			}
			moveNext(S);
		}
		for ( int j = 1; j <= initialLength; j++ ){
			deleteFront(S);
		}
